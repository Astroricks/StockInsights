      ...
      return (
        <div className="bg-background border border-border rounded-lg p-3 shadow-lg">
          ...
          <p className={`text-sm ${isPositive ? 'text-orange-600' : 'text-red-600'}`}>
            Net Income: {isPositive ? currencySymbol : '-' + currencySymbol}{Math.abs(value / 1000000).toFixed(0)}M
          </p>
        </div>
      );
    }
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className={`font-semibold ${latestNetIncome >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {latestNetIncome >= 0 ? (isUSD ? '$' : reportedCurrency + ' ') : '-' + (isUSD ? '$' : reportedCurrency + ' ')}{Math.abs(latestNetIncome / 1000000).toFixed(0)}M
              </p>
            </div>
            <div>
